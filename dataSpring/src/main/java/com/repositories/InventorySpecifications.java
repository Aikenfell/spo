...
public class InventorySpecifications {
  ...
  public static Specification<Inventory> dynamicName(String name) {
  ...
  public static Specification<Inventory> getByWarehouseID(Optional<Warehouse> warehouseID) {
    return (root, query, builder) -> {

        //   LocalDate date = LocalDate.now().minusYears(2);
    //   return builder.equal(root.get(entityOne.createdAt), date);
      return builder.equal(root.get("warehouse_id"), warehouseID);
    };
  }
